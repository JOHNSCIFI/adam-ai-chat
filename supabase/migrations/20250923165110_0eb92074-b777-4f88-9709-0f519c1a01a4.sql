...
-- Create policies for user_subscriptions (drop existing first)
...
-- Create favorite tools table (if not exists)
...
-- Create policies for favorite_tools (drop existing first)
DROP POLICY IF EXISTS "Users can view their own favorite tools" ON public.favorite_tools;
DROP POLICY IF EXISTS "Users can manage their own favorite tools" ON public.favorite_tools;
...
-- Create trigger for updated_at column on user_subscriptions (if not exists)
